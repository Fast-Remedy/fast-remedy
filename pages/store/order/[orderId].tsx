...
const Order: React.FC = () => {
	...
	const [isFinishMenuVisible, setIsFinishMenuVisible] = useState(false);
	...
	const handleAcceptOrder = async () => {
		...
		try {
			...
			newOrder.statusOrder = 'inProgress';
			setOrder(newOrder);
		} catch (error) {
			console.log(error);
		}
	...
	return (
		<Container>
			<>
				{!isCancelMenuVisible && !isFinishMenuVisible && (
					<motion.div
						initial={{ opacity: 0 }}
						exit={{ opacity: 0 }}
						animate={{ opacity: 1 }}
							...
							{isFetching ? (
								...
								<>
									<DetailsCard>
										<Text>
											<Customer>
												<Name>{order.customer.nameCustomer}</Name>
											</Customer>
											<Status>
												<Description>
													Status:
									...
									<BoxCard>
										...
										<FinishCard>
											<Info>
												...
												<span className='info'>
													{`${order.paymentOrder[0].cardTypeCustomers} - ${order.paymentOrder[0].processor}`}
												</span>
											</Info>
										</FinishCard>
										{order.statusOrder === 'pendingAcceptance' && (
											...
											<>
												...
												<FinishCard>
													<Button
														...
														color={Theme.colors.white}
														backgroundColor={Theme.colors.green}
														onClick={() => setIsFinishMenuVisible(true)}
													>
														<svg
															fill='currentColor'
															viewBox='0 0 20 20'
					...
					<AnimatePresence>
						...
						>
							<Section>
								...
								<CancelCard>
									<span>Deseja realmente recusar / cancelar o pedido?</span>
				...
				{!isCancelMenuVisible && isFinishMenuVisible && (
					<AnimatePresence>
						...
						>
							<Section>
								<TitleBox title='Finalizar Pedido' />
								<CancelCard>
									<span>Deseja realmente finalizar o pedido?</span>
									<div className='buttons'>
										<Button
											className='icon back'
											width='10rem'
											color={Theme.colors.black}
											backgroundColor={Theme.colors.gray}
											onClick={() => setIsFinishMenuVisible(false)}
										>
											<svg
												className='icon'
												fill='currentColor'
								...
								<AnimatePresence>
									{isMessageVisible && (
										<motion.div
											...
											transition={{ duration: 0.3 }}
										>
											<FinishMessage>Pedido finalizado!</FinishMessage>
										</motion.div>
									)}
								</AnimatePresence>
							</Section>
